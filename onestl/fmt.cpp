...
ONESTL_NOINLINE const Fmt &Fmt::write(const char *chars, size_t n) const noexcept {
  if (err_ == 0 && write_ != nullptr) {
    err_ = write_(handle_, chars, n);
  }
  return *this;
}

ONESTL_NOINLINE const Fmt &Fmt::append(char ch) const noexcept {
  return write(&ch, 1);
}

ONESTL_NOINLINE const Fmt &Fmt::append(int64_t val) const noexcept {
  if (val < 0) {
    write("-", 1);
    val = -val;
  }
...
ONESTL_NOINLINE const Fmt &Fmt::append(uint64_t val) const noexcept {
...
ONESTL_NOINLINE const Fmt &Fmt::append(double val) const noexcept {
  enum _ : int { cap = 24 };
  char buf[cap];
  int n = snprintf(buf, cap, "%g", val);
  if (n > cap) {
    n = cap;
  }
  return write(buf, n);
...
const Fmt &operator<<(const Fmt &fmt, bool val) noexcept {
...
static char to_hex_digit(uint8_t val) noexcept {
