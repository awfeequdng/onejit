...
func TestDeclConstTyped(t *testing.T) {
	p := makeParser(`const ( foo, bar *map[int]int )`)
...
func TestDeclVarTyped(t *testing.T) {
	p := makeParser(`var ( foo, bar <-chan <-chan int )`)
	compareNode(t, p.Parse(), `(var (VALUE_SPEC`+
		` (NAMES (IDENT foo) (IDENT bar))`+ // ident list
...
func TestDeclVarTypedInit(t *testing.T) {
	p := makeParser(`var ( x, y, z type1 = e1; w type2 = e2; )`)
...
func TestDeclVarTypedInitCompositeLit(t *testing.T) {
	p := makeParser(`var x typ = typ{a:1, b:2}`)
	compareNode(t, p.Parse(), `(var `+
...
func TestDeclTypeStructTag(t *testing.T) {
	p := makeParser(`type quirky = struct { int int "tag" } `) // field has name "int" and type "int"
...
func TestDeclTypeStructEmbed(t *testing.T) {
	p := makeParser(`type embed = struct { int; *float } `) // two embedded fields
...
func TestDeclTypeFunc(t *testing.T) {
	p := makeParser(`type StateMachine func(env *Env) (next StateMachine, newEnv *Env)`)
...
func TestDeclTypeFuncImpliedParamType(t *testing.T) {
	p := makeParser(`type add3 func(a int, b, c int) int`)
...
func TestDeclTypeFuncVariadic(t *testing.T) {
	p := makeParser(`type addv func(n0 int, nv... int) int`)
...
func TestDeclTypeGeneric(t *testing.T) {
	p := makeParser(`type pair[T1, T2 any] struct { first T1; second T2 }`)
...
func TestDeclFuncBad1(t *testing.T) {
	p := makeParser(`func bad1(a ... float, b float) { } `)
...
func TestDeclFuncEllipsis(t *testing.T) {
	p := makeParser(`func println(args ... interface{}) (int, error) { }`)
...
func TestDeclFuncArgIsArray(t *testing.T) {
	...
	p := makeParser(`func tostring(args[len]int) { }`)
...
func TestDeclFuncArgIsSlice(t *testing.T) {
	...
	p := makeParser(`func tostring(args []int) { }`)
	compareNode(t, p.Parse(), `(func nil (IDENT tostring) `+
		`(func`+
...
func TestDeclFuncArgIsGeneric(t *testing.T) {
	p := makeParser(`func tostring(pair[int,uint]) { }`)
	compareNode(t, p.Parse(), `(func nil (IDENT tostring) `+
		`(func`+
...
func TestDeclFuncArgIsFunc(t *testing.T) {
	p := makeParser(`func foo(func(), error) { }`)
...
func TestDeclFuncResultIsFunc(t *testing.T) {
	p := makeParser(`func foo(error) func() { }`)
	compareNode(t, p.Parse(), `(func nil (IDENT foo) `+
		`(func`+
...
func TestDeclMethodResults(t *testing.T) {
	p := makeParser(`func (p *Parser) fixParams() (a, b.c, d) { } `)
...
func TestExprCompositeLit1(t *testing.T) {
	p := makeParser(`T{key1:value1, {key2}:{value2}, value3}`)
...
func TestExprCompositeLit2(t *testing.T) {
	p := makeParser(`struct{ i int }{1}`)
	compareNode(t, p.Parse(), `(COMPOSITE_LIT `+
		`(struct (FIELD (NAMES (IDENT i)) (IDENT int))) `+
		`(INT 1))`)
}

func TestExprLiteralRune(t *testing.T) {
	p := makeParser(`'x' + '\xae' + '\uffff' + '\U0010ffff'`)
	compareNode(t, p.Parse(),
		`(+ (+ (+ (CHAR 'x') (CHAR '\xae')) (CHAR '\uffff')) (CHAR '\U0010ffff'))`)
}

func TestExprLiteralFloat(t *testing.T) {
	p := makeParser(`1.0 + 2. + .3 + 4e5 + .6e+7 + 8.9e-10`)
	compareNode(t, p.Parse(),
		`(+ (+ (+ (+ (+ (FLOAT 1.0) (FLOAT 2.)) (FLOAT .3)) (FLOAT 4e5)) (FLOAT .6e+7)) (FLOAT 8.9e-10))`)
}

func TestExprLiteralHexFloat(t *testing.T) {
	p := makeParser(`0x1p2`)
	compareNode(t, p.Parse(),
		`(FLOAT 0x1p2)`)
}
...
func TestStmtForInitCondPost(t *testing.T) {
	p := makeParser(`for i := init; i != 0; i-- { break; continue }`)
	compareNode(t, p.Parse(), `(for`+
...
func TestStmtForInitCondPostCompositeLit(t *testing.T) {
	p := makeParser(`for init; cond; (post{key:value}) {  }`)
	compareNode(t, p.Parse(), `(for`+
		` (IDENT init)`+
		` (IDENT cond)`+
...
func TestStmtIfInitCond(t *testing.T) {
	p := makeParser(`if init; cond { b } else if c { d }`)
...
func TestStmtGoto(t *testing.T) {
	p := makeParser(`loop: goto loop`)
	compareNode(t, p.Parse(), `(LABEL (IDENT loop) (goto (IDENT loop)))`)
}

func TestStmtSelect(t *testing.T) {
	p := makeParser(`select { case a <- b: foo; case <- c: bar; case v,ok := <- a: baz; default: etc }`)
...
func TestStmtSwitchType(t *testing.T) {
	p := makeParser(`switch x := a.b.(type) {  }`)
...
func TestTrickyExprRecv(t *testing.T) {
	p := makeParser(`channel <- *arr[typ{}.field]`)
	compareNode(t, p.Parse(),
		`(<- (IDENT channel) (* (INDEX (IDENT arr) (. (COMPOSITE_LIT (IDENT typ)) (IDENT field)))))`)
}

func TestTrickyStmtFor(t *testing.T) {
	// 'x {' does not start a composite literal
	p := makeParser(`for ; ; channel <- x {	}`)
	compareNode(t, p.Parse(), `(for nil nil (<- (IDENT channel) (IDENT x)) (BLOCK))`)
}

func TestTrickyStmtSwitch(t *testing.T) {
	// 'switch ; { }' is valid
	p := makeParser(`switch ; { }`)
	compareNode(t, p.Parse(), `(switch nil nil)`)
}
